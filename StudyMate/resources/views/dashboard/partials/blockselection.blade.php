    ...
    @for($i = 1; $i<=4;$i++)
        <div class="btn-group" role="group">
            ...
            <div class="dropdown-menu">
                <a href="{{ route('getDashboardDetails', ['block' => $i]) }}" class="text-decoration-none">
                    <button type="button" class="btn dropdown-item" id="btn{{$i}}">Block {{$i}}</button>
                </a>
                <a href="{{ route('getDashboardDetails', ['block' => $i+4]) }}" class="text-decoration-none">
                    <button type="button" class="btn dropdown-item btn-info" id="btn{{$i+4}}">Block {{$i+4}}</button>
                </a>
                <a href="{{ route('getDashboardDetails', ['block' => $i+8]) }}" class="text-decoration-none">
